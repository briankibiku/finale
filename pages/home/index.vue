          ...
          <div>
            ...
            >
              <div v-for="round in roundsScoreList">
                ...
                >
                  <div>
                    <span class="points-heading">{{ round.score * 10 }}</span
                    >pts
                  </div>
                  <div class="round-title">Round {{ round.row }}</div>
                </div>
...
import { consoleSandbox, CONSOLE_LEVELS } from '@sentry/utils';
export default {
  data() {
    return {
  ...
  methods: {
    ...
    forceRerender() {
      ...
      //   ? (this.customBtnText = "Session is live: Play Now & Win")
      //   : (this.customBtnText = "Game Not Live");
      // this.istournamentLive
    ...
    async getRoundScore() {
      try {
        ...
        // this.remainingRounds = 4 - parseInt(this.roundsScoreList.length);
      } catch (error) {
        this.errorToast();
    ...
    async getTournamentDetails() {
      ...
      // tournamentLive === 1 && (this.istournamentLive = true);
      // tournamentLive === 0 && (this.istournamentLive = false);
